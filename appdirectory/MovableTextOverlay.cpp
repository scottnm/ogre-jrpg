...
{
    const Ogre::Font *pFont = mAttrs->getFont();
    mTextWidth = 0;
...
void MovableTextOverlay::_getMinMaxEdgesOfTopAABBIn2D(Ogre::Real& MinX, Ogre::Real& MinY, Ogre::Real& MaxX, Ogre::Real& MaxY)
{
    ...
    Ogre::Plane CameraPlain = Ogre::Plane(CameraPlainNormal,mpCam->getDerivedPosition());//the plaine that devides the space bevor and behin the cam
...
{
    const Ogre::Camera* mpCam = mAttrs->getCamera();
    Ogre::Vector3 hcsPosition = mpCam->getProjectionMatrix() * (mpCam->getViewMatrix() * position);
...
{
    static Ogre::Real timeTillUpdate = 0;
    ...
    Ogre::Real relTextHeight = mAttrs->getCharacterHeight() / Ogre::OverlayManager::getSingleton().getViewportHeight();
...
{
    const Ogre::String &newMatName = mAttrs->getMaterialName();
    const Ogre::String &oldMatName = mpOvContainer->getMaterialName();
